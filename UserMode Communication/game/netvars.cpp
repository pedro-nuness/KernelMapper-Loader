...
auto c_netvars::init() -> bool {
	...
	auto get_list = Memory->FindPattern2(Global->Client, "A1 ? ? ? ? 8B ? 0C 85 C0 74 18");
	
	if (!get_list)
		return false;

	auto list = Global->K->ReadMemory<uintptr_t>(Global->K->ReadMemory<uintptr_t>(get_list + 0x1));

	for (auto client_class = list; !!client_class; client_class = Global->K->ReadMemory<uintptr_t>(client_class + offsetof(struct ClientClass, m_pNext))) {
		auto recv_table = Global->K->ReadMemory<uintptr_t>(client_class + offsetof(struct ClientClass, m_pRecvTable));
		auto table_name = Global->K->ReadMemory<uintptr_t>(recv_table + offsetof(struct RecvTable, m_pNetTableName));
		
		char name[64]{};
		Global->K->Read(table_name, name, sizeof(name));
...
RecvTable GetToRecv(ULONG64 adress)
{
	void* tableptr = Global->K->ReadMemory<PVOID>(adress);
	RecvTable* tab = (RecvTable*)tableptr;
	return *tab;
}
...
auto c_netvars::get_entry(std::string name, uintptr_t table_ptr, uintptr_t offset/* = 0*/) const -> std::ptrdiff_t {
	...
	auto table = GetToRecv(table_ptr);
	...
	Global->K->Read(table.m_pProps, props, props_size);

	for (int i{}; i < table.m_nProps; ++i) {
		RecvProp prop = props[i];

		char prop_name[64]{};
		Global->K->Read(prop.m_pVarName, prop_name, sizeof(prop_name));

		if (isdigit(prop_name[0]))
			continue;

		if (prop.m_pDataTable) {
			auto child = GetToRecv(prop.m_pDataTable);
			if (child.m_nProps)
			{
...
auto c_netvars::dump_table(std::ostream& stream, uintptr_t table_ptr, uintptr_t offset/* = 0*/, int level/* = 0*/) const -> void
{
	...
	char name[64]{};
	Global->K->Read(table.m_pNetTableName, name, sizeof(name));
