...
void Entity::Update(int iIndex)
{
	...
	m_dwPointer =  Global->K->ReadMemory<DWORD_PTR>(Global->Client + ofs->m_dwEntityList + m_iIndex * 0x10);

	if (!m_dwPointer)
		return;

	//    Read Player Chunk    //
	Global->K->Read(m_dwPointer, chunk, sizeof(chunk));
	...
	if (!m_dwBoneMatrix)
	 ...
	 Global->K->Read(m_dwBoneMatrix, m_BoneMatrix, sizeof(Matrix3x4[128]));
...
{
	short weaponId = Global->K->ReadMemory<short>(GetActiveWeaponBase() + ofs->m_iItemDefinitionIndex);
...
DWORD_PTR Entity::GetActiveWeaponBase()
{
	...
	return  Global->K->ReadMemory<DWORD_PTR>(Global->Client + ofs->m_dwEntityList + (dwBaseCombatWeaponIndex - 1) * 0x10);
}

float Entity::GetAccuracyPenalty()
{
	return  Global->K->ReadMemory<float>(GetActiveWeaponBase() + 0x32C0);
}
...
int Entity::GetActiveWeaponAmmo()
{
	...
	return  Global->K->ReadMemory<uint32_t>(WeaponBase + ofs->m_iClip1);
}
...
player_info_t Entity::GetProfileInfo()
{
	...
	uintptr_t ecx = Global->K->ReadMemory<uintptr_t>(ofs->m_dwClientState + ofs->m_dwClientState_PlayerInfo);
	uintptr_t edx = Global->K->ReadMemory<uintptr_t>(ecx + ECX_DISP);
	uintptr_t eax = Global->K->ReadMemory<uintptr_t>(edx + EDX_DISP);
	uintptr_t profile_info = Global->K->ReadMemory<uintptr_t>(eax + INFO_OFFSET + (this->GetIndex() - 1) * ENTRY_SIZE);

	void* sec = Global->K->ReadMemory<void*>(profile_info);

	player_info_t* Mem = (player_info_t*)sec;
	return *Mem;
}

int Entity::GetRank()
{
	uintptr_t GameResources =  Global->K->ReadMemory<uintptr_t>(Global->Client + ofs->m_dwPlayerResource);
	return  Global->K->ReadMemory<uint32_t>(GameResources + ofs->m_iCompetitiveRanking + this->GetIndex() * 4);
}

int Entity::GetWins()
{
	uintptr_t GameResources =  Global->K->ReadMemory<uintptr_t>(Global->Client + ofs->m_dwPlayerResource);
	return  Global->K->ReadMemory<uint32_t>(GameResources + ofs->m_iCompetitiveWins + (this->GetIndex() * 4));
}
