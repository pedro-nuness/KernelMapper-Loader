...
void bStart()
{
	...
	std::cout << "[+] Waiting for game PID!\n";

	while (!Game)
	{
		Game = GetProcessPID("cod.exe");
		Sleep(200);
	}

	std:cout << "Game PID: " << Game << std::endl;
	std::cout << "[+] Found game!\n";
	std::cout << "[+] Setupping game!\n";
...
)
{
	...
	case DLL_PROCESS_ATTACH:
		CloseHandle(CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)bMain, hModule, NULL, NULL));
		break;
